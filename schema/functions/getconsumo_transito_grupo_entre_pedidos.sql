CREATE OR REPLACE FUNCTION public.getconsumo_transito_grupo_entre_pedidos(p_grupo numeric, idprod character varying)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
...
begin 
       ...
       select count(*) into p_atraso  from analise_requisicoes ar  
            where filial in(select filial from grupo_filial gf where gf.id_grupo=p_grupo)  
...
from
    ...
    select
       ...
       (current_date+produtos.tempo_medio_ressuprimento::int4) - (current_date + ((vw_requisicoes.data_solicitacao+produtos.tempo_medio_ressuprimento::int4) - current_date))   as  amplitude
