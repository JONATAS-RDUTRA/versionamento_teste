CREATE OR REPLACE FUNCTION public.processar_analise_diagnostico_estoque_grupo_data(p_data date)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
begin
    ...
    FOR rec_produto IN (
        ...
        pedidos_em_aberto as (
            ...
            where r.qtde_pendente > 0 AND r.data_solicitacao <= p_data
        ...
        produtos_por_grupo AS (
            SELECT 
                ...
                COALESCE(avg(NULLIF(pf.custo_unitario, 0::numeric)) FILTER (WHERE (sf.estoque) > 0), 0::numeric) AS custo_unitario,
                COALESCE(avg(NULLIF(pf.valor_unitario, 0::numeric)), 0::numeric) AS valor_unitario,
                sum(sf.estoque) AS estoque,
                sum(sf.esseg) AS estoque_seguranca,
                sum(sf.ppd) AS ponto_pedido,
                sum(sf.emax) AS estoque_maximo,
                sum(sf.cmm) AS consumo_medio_mensal,
                COALESCE(min(substring(sf.arvore_decisao, 1, 1)) || max(substring(sf.arvore_decisao, 2, 1)) || min(substring(sf.arvore_decisao, 3, 1)) || min(substring(sf.arvore_decisao, 4, 1)), 'CX2R'::text) AS arvore_decisao,
                sum(pf.tempo_medio_apanhe) AS tempo_medio_apanhe,
                getnivelservico(COALESCE(min(substring(sf.arvore_decisao, 1, 1)) || max(substring(sf.arvore_decisao, 2, 1)) || min(substring(sf.arvore_decisao, 3, 1)) || min(substring(sf.arvore_decisao, 4, 1)), 'CX2R'::character varying)) AS nivel_servico,
                max(pf.revenda) AS revenda,
                min(pf.status) AS status,
                avg(sf.tempo_ressuprimento) AS tempo_ressuprimento,
                avg(sf.tempo_ressuprimento * 30) AS tempo_medio_ressuprimento,
                COALESCE(trunc(sum(sf.emax * pf.custo_unitario), 4), 0) AS emax_em_preco_custo,
                COALESCE(trunc(sum((sf.estoque) * pf.custo_unitario), 4), 0) AS estoque_em_preco_custo,
                COALESCE(trunc(sum((sf.estoque) * pf.valor_unitario), 4), 0) AS estoque_em_preco_venda,
                COALESCE(trunc(sum(sf.cmm * pf.preco_medio_venda), 4), 0) AS cmm_em_preco_venda,
                COALESCE(trunc(sum(sf.cmm * pf.custo_unitario), 4), 0) AS cmm_em_preco_custo,
                trunc(sum(GREATEST((sf.estoque) - sf.emax, 0) * pf.custo_unitario), 4) AS estoque_em_excesso_em_preco_custo,
                sum(pf.estoque_minimo) AS estoque_minimo,
                sum(sf.sugestao_compras) AS sugestao_compra,
                min(pf.classificacao_rentabilidade) AS classificacao_rentabilidade
            FROM saldo_filiais sf 
                INNER JOIN produtos_filial pf ON pf.filial = sf.filial AND pf.idproduto = sf.idproduto 
                INNER JOIN grupo_filial gf ON gf.filial = pf.filial
                INNER JOIN grupo_compras gc ON gc.id = gf.id_grupo AND gc.grupo_agrega <> 'S'
            WHERE 
                sf.data = p_data
                AND sf.idproduto NOT IN (SELECT pd.idproduto FROM produtos_desconsiderados_analise_diagnostico_estoque pd)
            GROUP BY gf.id_grupo, pf.idproduto, pf.descricao_produto
        ),
        produtos_classificados AS (
            SELECT
                ...
                (p_data - us.ult_saida) AS qtde_dias_sem_vender,
                ...
                COALESCE(pf.sugestao_compra, 0) AS sugestao_compra,
                COALESCE(pf.sugestao_compra * pf.custo_unitario, 0) AS sugestao_compra_preco_custo,
        ...
        v2 AS (
            SELECT
                ...
                CASE
                    ...
                    WHEN (p_data - v1.data_cadastro) < 90 THEN '1. Menor que 90 dias'
                    WHEN (p_data - v1.data_cadastro) <= 180 THEN '2. Entre 90 dias a 180 dias'
