CREATE OR REPLACE FUNCTION public.getconsumo_transito_grupo(p_grupo numeric, idprod character varying)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
...
from
	...
	select
	  ...
	  (current_date - vw_requisicoes.data_solicitacao)  as  amplitude
   ...
   on  produtos.filial =vw_requisicoes.filial and  produtos.idproduto = vw_requisicoes.idproduto::varchar
    ...
    amp= greatest((tempo_ressuprimento - amp),0);
