CREATE OR REPLACE FUNCTION public.get_calculo_dimensoes_estoque_filial(p_filial numeric, idprod character varying, arv_decisao character varying, dataref date DEFAULT ('now'::text)::date)
...
begin
  ...
  select
	...
	coalesce( sf.estoque,pf.estoque),
  ...
  from produtos_filial pf
   ...
   left join saldo_filiais sf 

     on sf.filial = pf.filial

     and sf.idproduto = pf.idproduto

     and sf."data" = dataref

  where  pf.filial= p_filial and pf.idproduto = idprod;
