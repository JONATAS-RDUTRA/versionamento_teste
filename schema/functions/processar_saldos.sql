CREATE OR REPLACE FUNCTION public.processar_saldos(dataini date, datafim date)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
declare
...
begin
  ...
  saida = gerar_saldos_diario(dataini,datafim); 

  for rec_prod in (select idproduto from produtos order by idproduto)
  loop
