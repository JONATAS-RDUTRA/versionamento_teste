CREATE OR REPLACE FUNCTION public.gerar_saldos_diario(p_filial integer, dataini date, datafim date, mov numeric DEFAULT 0)
...
begin
  ...
  if mov = 0 then 
      ...
      for rec_prod in (select filial,idproduto,tempo_ressuprimento,desvio_padrao_ressuprimento,data_cadastro,
                        ...
                        from produtos_filial pf where (pf.filial,pf.idproduto) in (
                          select distinct filial ,idproduto from consumos c where filial = p_filial and emissao between current_date - 1095 and current_date
							union 
						  select distinct idfilial ,idproduto from entrada_mercadorias em  where idfilial = p_filial and data_entrada  between current_date - 1095 and current_date
						    union 
						  select filial,idproduto from hist_estoque he  where filial =p_filial and "data" between dataini and datafim and qtde > 0  
                         ) 
        ...
        for rec_prod in (select filial,idproduto,tempo_ressuprimento,desvio_padrao_ressuprimento,data_cadastro,
							...
							select distinct filial ,idproduto from consumos c where filial = p_filial and emissao between dataini and datafim
							union 
							select distinct idfilial ,idproduto from entrada_mercadorias em  where idfilial = p_filial and data_entrada  between dataini and datafim
							)
							order by idproduto,filial)
