CREATE OR REPLACE FUNCTION public.getconsumo_transito_grupo_filial_new(p_grupo integer, p_filial integer, idprod character varying)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
declare 
...
begin 
   ...
   select amplitude into amp
	from (
	    select (vw_requisicoes.data_solicitacao+produtos.tempo_medio_ressuprimento::int4) - current_date  as  amplitude
	    from vw_requisicoes
