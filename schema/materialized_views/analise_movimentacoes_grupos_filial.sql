...
\restrict Vc35rcbWh9tfIAO0OKITJ7Q7KHn95livj9DWEfnc5vODqsbc1swzzmGwhm7ggUF
...
CREATE MATERIALIZED VIEW public.analise_movimentacoes_grupos_filial AS
 SELECT 'SD'::text AS tipo,
  ...
  GROUP BY (to_char((c.emissao)::timestamp with time zone, 'YYYY'::text)), (to_char((c.emissao)::timestamp with time zone, 'MM'::text)), p.idfornecedor, f.razao_social, p.filial, g.id_grupo
UNION ALL
 SELECT 'RQ'::text AS tipo,
  ...
  GROUP BY (to_char((r.data_solicitacao)::timestamp with time zone, 'YYYY'::text)), (to_char((r.data_solicitacao)::timestamp with time zone, 'MM'::text)), p.idfornecedor, f.razao_social, p.filial, g.id_grupo
UNION ALL
 SELECT 'EN'::text AS tipo,
  ...
  GROUP BY (to_char((m.data_entrada)::timestamp with time zone, 'YYYY'::text)), (to_char((m.data_entrada)::timestamp with time zone, 'MM'::text)), p.idfornecedor, f.razao_social, p.filial, g.id_grupo
  WITH NO DATA;
...
\unrestrict Vc35rcbWh9tfIAO0OKITJ7Q7KHn95livj9DWEfnc5vODqsbc1swzzmGwhm7ggUF
