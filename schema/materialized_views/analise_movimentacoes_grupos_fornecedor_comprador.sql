...
\restrict j67xywiZHvQnhPBMcPynUibhjKQGVDUXhTdrWKZ5ynpJrybk0sKbA8njaJGRliO
...
CREATE MATERIALIZED VIEW public.analise_movimentacoes_grupos_fornecedor_comprador AS
 SELECT 'SD'::text AS tipo,
  ...
  GROUP BY (to_char((c.emissao)::timestamp with time zone, 'YYYY'::text)), (to_char((c.emissao)::timestamp with time zone, 'MM'::text)), p.idfornecedor, f.razao_social, g.id_grupo, p.idcomprador
UNION ALL
 SELECT 'RQ'::text AS tipo,
  ...
  GROUP BY (to_char((r.data_solicitacao)::timestamp with time zone, 'YYYY'::text)), (to_char((r.data_solicitacao)::timestamp with time zone, 'MM'::text)), p.idfornecedor, f.razao_social, g.id_grupo, p.idcomprador
UNION ALL
 SELECT 'EN'::text AS tipo,
  ...
  GROUP BY (to_char((m.data_entrada)::timestamp with time zone, 'YYYY'::text)), (to_char((m.data_entrada)::timestamp with time zone, 'MM'::text)), p.idfornecedor, f.razao_social, g.id_grupo, p.idcomprador
  WITH NO DATA;
...
\unrestrict j67xywiZHvQnhPBMcPynUibhjKQGVDUXhTdrWKZ5ynpJrybk0sKbA8njaJGRliO
